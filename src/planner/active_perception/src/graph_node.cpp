...
double ViewNode::computePathLen(const Eigen::Vector3d& pose1, const Eigen::Vector3d& pose2)
{
  Vector3d pos1 = pose1;
  Vector3d pos2 = pose2;
  vector<Vector3d> path;
  double path_cost = ViewNode::searchPath(pos1, pos2, path);
  return path_cost;
}

double ViewNode::computeCostPY(const Eigen::VectorXd& pose1, const Eigen::VectorXd& pose2)
{
  // Cost of position change
  Vector3d pos1 = pose1.head(3);
  Vector3d pos2 = pose2.head(3);
  vector<Vector3d> path;
  double pos_cost = ViewNode::searchPath(pos1, pos2, path) / vm_;

  // Cost of pitch change
  double pitch1 = pose1(3);
  double pitch2 = pose2(3);
  double diff_pitch = fabs(pitch1 - pitch2);
  double pitch_cost = diff_pitch / yd_;

  // Cost of yaw change
  double yaw1 = pose1(4);
  double yaw2 = pose2(4);
  double diff_yaw = fabs(yaw1 - yaw2);
  diff_yaw = min(diff_yaw, 2 * M_PI - diff_yaw);
  double yaw_cost = diff_yaw / yd_;
  return max(pos_cost, max(pitch_cost, yaw_cost));
}
}  // namespace hetero_planner