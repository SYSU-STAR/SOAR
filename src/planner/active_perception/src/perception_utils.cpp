...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh, int mission_type)
{
  ...
  nh.param("perception_utils/camera_top_angle", camera_top_angle_, -1.0);
  nh.param("perception_utils/camera_bottom_angle", camera_bottom_angle_, -1.0);
  nh.param("perception_utils/camera_left_angle", camera_left_angle_, -1.0);
  nh.param("perception_utils/camera_right_angle", camera_right_angle_, -1.0);
  nh.param("perception_utils/camera_max_dist", camera_max_dist_, -1.0);
  ...
  if (mission_type == 0) {
    ...
    top_angle_ = camera_top_angle_;
    bottom_angle_ = camera_bottom_angle_;
    left_angle_ = camera_left_angle_;
    right_angle_ = camera_right_angle_;
    max_dist_ = camera_max_dist_;
    vis_dist_ = camera_vis_dist_;
  }
  ...
  double vert_top = vis_dist_ * tan(top_angle_);
  double vert_bottom = vis_dist_ * tan(bottom_angle_);
  Vector3d origin(0, 0, 0);
  Vector3d left_up(vis_dist_, hor, vert_top);
  Vector3d left_down(vis_dist_, hor, -vert_bottom);
  Vector3d right_up(vis_dist_, -hor, vert_top);
  Vector3d right_down(vis_dist_, -hor, -vert_bottom);
