...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt)
{
  ...
  while (!open_set_.empty()) {
    ...
    if ((ros::Time::now() - t1).toSec() > max_search_time_) {
      // std::cout << "early";
      ROS_INFO("A star long time [max_time is %f]", max_search_time_);
  ...
  // ROS_ERROR("run out | close_set_map_ = %ld start inf_occ = %d start occ = %d",
  //     close_set_map_.size(), edt_env_->sdf_map_->getInflateOccupancy(start_pt),
  //     edt_env_->sdf_map_->getOccupancy(end_pt));
  return NO_PATH;
}
