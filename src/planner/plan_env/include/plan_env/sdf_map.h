...
public:
  ...
  void inputGlobalMap(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num);
  ...
  int getGlobalOccupancy(const Eigen::Vector3d& pos);
  int getGlobalOccupancy(const Eigen::Vector3i& id);
  ...
  void getAllBox(Eigen::Vector3d& bmin, Eigen::Vector3d& bmax);
  double getResolution();
  int getVoxelNum();
  vector<Eigen::Vector3i> allNeighbors(const Eigen::Vector3i& voxel);
  vector<Eigen::Vector3i> getConnectedOccupiedCells(const Eigen::Vector3d& pos);

  void setSeen(const Eigen::Vector3i& idx);
...
private:
  ...
  bool is_lidar_;
  ros::Publisher lidar_local_update_bound_pub;
  ros::Subscriber camera_local_update_bound_sub;
  void cameraLocalUpdateBoundCallback(const std_msgs::Int32MultiArrayConstPtr& msg);
...
struct MapData {
  ...
  std::vector<char> global_occ_;
  std::vector<char> seen_occ_;
  std::vector<char> exp_occ_;
  // data for updating
  vector<short> count_hit_, count_miss_, count_hit_and_miss_;
  vector<char> flag_rayend_, flag_visited_;
  vector<Eigen::Vector3i> visited_idxs;
  char raycast_num_;
  ...
  vector<Eigen::Vector3i> lidar_update_bound_;
  bool camera_local_update_bound_ = false;
  bool is_start_map_ = false;
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};
...
inline int SDFMap::getGlobalOccupancy(const Eigen::Vector3i& id)
{
  if (!isInMap(id))
    return -1;
  if (md_->global_occ_[toAddress(id)])
    return OCCUPIED;
  return FREE;
}

inline int SDFMap::getGlobalOccupancy(const Eigen::Vector3d& pos)
{
  Eigen::Vector3i id;
  posToIndex(pos, id);
  return getGlobalOccupancy(id);
}
...
inline void SDFMap::setSeen(const Eigen::Vector3i& idx)
{
  if (!isInBox(idx))
    return;
  md_->seen_occ_[toAddress(idx)] = 1;
}
