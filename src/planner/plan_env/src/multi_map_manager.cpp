...
void MultiMapManager::init()
{
  ...
  drone_id_ += 1;
  // ROS_ERROR("DDDrone_id %d",drone_id_);
...
void MultiMapManager::updateMapChunk(const vector<uint32_t>& adrs)
{
  ...
  if (adr_buffer_.size() >= chunk_size_) {
    ...
    // std::cout << "" << std::endl;
    // std::cout << "chunk size: " << multi_map_chunks_[drone_id_ - 1].chunks_.size() <<
    // std::endl;
...
{
  // ROS_ERROR("DDDrone_id %d",drone_id_);
  if (msg->from_drone_id == drone_id_)
    return;
  // if (msg->from_drone_id == 1)
  // {
  //   // ROS_ERROR("MultiMapManager::stampMsgCallback");
  //   return;
  // }
  // else{
  //   // ROS_ERROR("222");
  ...
  if (msg->time - last_chunk_stamp_time_[msg->from_drone_id - 1] < 0.3)
    return;
  ...
  for (int i = 0; i < multi_map_chunks_.size(); ++i) {
    ...
    // ROS_ERROR("DDDrone_id %d",missed.size());
  }
    ...
    const int& chunk_drone_id, const int& to_drone_id, const vector<int>& idx_list)
{
  ...
  for (int i = 0; i < idx_list.size(); i += 2) {
    for (int j = idx_list[i]; j <= idx_list[i + 1]; ++j) {
      ...
      //           << int(msg.chunk_drone_id) << " to drone " << int(msg.to_drone_id) <<
      //           std::endl;
    }
  }
...
void MultiMapManager::get_lidar_ChunkBoxes(Eigen::Vector3d& min, Eigen::Vector3d& max, bool reset)
{
  min = chunk_boxes_[0].min_;
  max = chunk_boxes_[0].max_;
}
