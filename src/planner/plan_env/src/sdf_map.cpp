...
void SDFMap::initMap(ros::NodeHandle& nh)
{
  ...
  nh.param("frontier/is_lidar", is_lidar_, false);
  lidar_local_update_bound_pub =
      nh.advertise<std_msgs::Int32MultiArray>("/sdf_map/lidar_local_update_bound", 10);
  camera_local_update_bound_sub = nh.subscribe(
      "/sdf_map/lidar_local_update_bound", 10, &SDFMap::cameraLocalUpdateBoundCallback, this);
  ...
  md_->flag_visited_ = vector<char>(buffer_size, 0);
  ...
  md_->global_occ_ = vector<char>(buffer_size, 0);
  md_->seen_occ_ = vector<char>(buffer_size, 0);
  md_->exp_occ_ = vector<char>(buffer_size, 0);
...
{
  Eigen::Vector3i min_esdf;
  Eigen::Vector3i max_esdf;
  if (is_lidar_) {
    min_esdf = md_->local_bound_min_;
    max_esdf = md_->local_bound_max_;
    std::vector<Eigen::Vector3i> ESDF_local_bound;

    ESDF_local_bound.push_back(min_esdf);
    ESDF_local_bound.push_back(max_esdf);

    std_msgs::Int32MultiArray msg;
    for (const auto& vector_elem : ESDF_local_bound) {
      msg.data.push_back(vector_elem.x());
      msg.data.push_back(vector_elem.y());
      msg.data.push_back(vector_elem.z());
    }
    lidar_local_update_bound_pub.publish(msg);
  }
  else if (md_->camera_local_update_bound_) {
    min_esdf = md_->lidar_update_bound_[0];
    max_esdf = md_->lidar_update_bound_[1];
    // md_->camera_local_update_bound_ = false;
    md_->local_bound_min_ = min_esdf;
    md_->local_bound_max_ = max_esdf;
  }
  if (mp_->optimistic_) {
    ...
    const Eigen::Vector3d& camera_pos)
{
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    if (isInBox(idx)) {
      if (md_->global_occ_[vox_adr] == 1) {
        md_->exp_occ_[vox_adr] = 1;
      }
    }
  ...
  md_->is_start_map_ = true;
  // ROS_ERROR("new_voxel_ids_.size %d", new_voxel_ids_.size());
}
...
void SDFMap::inputGlobalMap(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num)
{
  if (point_num == 0)
    return;
  Eigen::Vector3d pt_w;
...
void SDFMap::getAllBox(Eigen::Vector3d& bmin, Eigen::Vector3d& bmax)
{
  bmin = md_->all_min_;
  bmax = md_->all_max_;
}
...
vector<Eigen::Vector3i> SDFMap::allNeighbors(const Eigen::Vector3i& voxel)
{
...
vector<Eigen::Vector3i> SDFMap::getConnectedOccupiedCells(const Eigen::Vector3d& pos)
{
  Eigen::Vector3i idx;
  posToIndex(pos, idx);

  int voxel_num = getVoxelNum();
  vector<char> visited_cell_flag_ = vector<char>(voxel_num, 0);
  fill(visited_cell_flag_.begin(), visited_cell_flag_.end(), 0);

  vector<Eigen::Vector3i> connected_occupied_cells;
  connected_occupied_cells.clear();
  ...
  while (!cell_queue.empty()) {
    ...
    cell_queue.pop();
    connected_occupied_cells.push_back(cur);

    auto nbrs = allNeighbors(cur);
    for (auto nbr : nbrs) {
      int adr = toAddress(nbr);
      if (!isInBox(nbr) || getOccupancy(nbr) != SDFMap::OCCUPIED || visited_cell_flag_[adr] == 1)
        continue;
      visited_cell_flag_[adr] = 1;
      cell_queue.push(nbr);
    }
  }

  return connected_occupied_cells;
}

void SDFMap::cameraLocalUpdateBoundCallback(const std_msgs::Int32MultiArrayConstPtr& msg)
{
  if (is_lidar_) {
    return;
  }
  md_->camera_local_update_bound_ = true;
  md_->lidar_update_bound_.resize(2);
  md_->lidar_update_bound_[0] = (Eigen::Vector3i(msg->data[0], msg->data[1], msg->data[2]));
  md_->lidar_update_bound_[1] = (Eigen::Vector3i(msg->data[3], msg->data[4], msg->data[5]));
}
