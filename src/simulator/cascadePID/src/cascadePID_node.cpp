...
void fuel_position_cmd_callback(
  ...
  acc_des = Eigen::Vector3d(cmd->acceleration.x, cmd->acceleration.y,
                            cmd->acceleration.z);
...
void run_control(const ros::TimerEvent &event) {
  if (first_odom_flag == 1) {
    ...
    Eigen::Quaterniond quaternion_temp(current_odom.pose.pose.orientation.w,
    ...
    current_pos << current_odom.pose.pose.position.x,
        current_odom.pose.pose.position.y, current_odom.pose.pose.position.z;
    current_vel << current_odom.twist.twist.linear.x,
        current_odom.twist.twist.linear.y, current_odom.twist.twist.linear.z;
    quad_PID.FeedbackInput(current_pos, current_vel, quaternion_temp);
    // ROS_INFO("current_pos =
    // %f,%f,%f",current_pos(0),current_pos(1),current_pos(2));
    ...
    if (control_flag == 1) {
      pos_des << pose_cmd.pose.position.x, pose_cmd.pose.position.y,
          pose_cmd.pose.position.z;

      Eigen::Quaterniond q_des(
          pose_cmd.pose.orientation.w, pose_cmd.pose.orientation.x,
    ...
    ROS_INFO("Torque_des = %f,%f,%f", Torque_des(0), Torque_des(1),
             Torque_des(2));
    Vector4d RPM_output;
    RPM_output = quad_PID.getRPMoutputs();
    // ROS_INFO("RPM_output =
    // %f,%f,%f,%f",RPM_output(0),RPM_output(1),RPM_output(2),RPM_output(3));
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber position_cmd_sub_ =
      n.subscribe("position_cmd", 10, fuel_position_cmd_callback,
                  ros::TransportHints().tcpNoDelay());

  quad_PID.setdroneid(drone_id);

  ros::Timer controller_timer =
      n.createTimer(ros::Duration(1.0 / controller_rate), run_control);

  Matrix3d Internal_mat;
  Internal_mat << 2.64e-3, 0, 0, 0, 2.64e-3, 0, 0, 0, 4.96e-3;
